...
char ch[34] = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ", cH[34] = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ", autoList[34] = "оеаинтсрвлкмдпуяыьгзбчйхжшюцщэфъё", chastot[34] = "ОЕАИТНСРВЛ", CH[34] = "абвгдеёжзийклмнопрстуфхцчшщъыьэюя", zamen[34], words[409600][100], tekst[409600], backCh1[1000], backCh2[1000];
...
int main()
{
	...
	int kom = 0;
	while (1) {
		printf("Input command (begining - -1, outAllWord - 0, outByUnChip - 1, outStat - 2, outText - 3, ChangeChar - 4, backChar - 5, Отмена замены символа - 6, outKey - 7, saveText - 8, autoChipher - 9):");
		scanf("%d", &kom);
		if (kom == -1) input();
		if (kom == 0) outAllWord();
